...
StereoRectifier::StereoRectifier(const cv::Size &img_size,
                                 const CalibStereo &calibration) {
  ...
  cv::stereoRectify(K_l, D_l, K_r, D_r, img_size, calibration.right_R_left,
                    calibration.right_t_left, R_l, R_r, P_l, cam_rect_matrix_,
                    reproj_mat_, cv::CALIB_ZERO_DISPARITY, 0, img_size);
    ...
    : StereoRectifier(
          ...
          CalibStereo(
              CalibMono(yaml_node["Calibration.left.fx"].as<double>(),
                        ...
                        yaml_node["Calibration.left.distortion"]
              ...
              CalibMono(yaml_node["Calibration.right.fx"].as<double>(),
                        ...
                        yaml_node["Calibration.right.distortion"]
                            .as<std::vector<double>>()),
              RotationVecToMat(ParseVectorAsMat(
                  yaml_node["Calibration.rotation"].as<std::vector<double>>())),
              ParseVectorAsMat(yaml_node["Calibration.translation"]
                                   .as<std::vector<double>>())
                  .t())) {}

StereoRectifier::~StereoRectifier() {
  spdlog::debug("DESTRUCT: StereoRectifier");
}

void StereoRectifier::rectify(const cv::Mat &in_img_l, const cv::Mat &in_img_r,
                              cv::Mat &out_img_l, cv::Mat &out_img_r) const {
  cv::remap(in_img_l, out_img_l, undist_map_x_l_, undist_map_y_l_,
            cv::INTER_LINEAR);
  cv::remap(in_img_r, out_img_r, undist_map_x_r_, undist_map_y_r_,
            cv::INTER_LINEAR);
}

cv::Mat StereoRectifier::RectifiedIntrinsics() const {
  return cam_rect_matrix_;
}

cv::Mat
StereoRectifier::GetIntrinsicsAsMat(const CalibMono &calibration) const {
