...
// this is preferred
template <typename UNIT> inline int64_t GetTimestamp() {
...
template <typename UNIT> inline int64_t GetSystemTimestamp() {
  const auto tp = std::chrono::system_clock::now().time_since_epoch();
  return std::chrono::duration_cast<UNIT>(tp).count();
}

template <typename UNIT> class LocalClock {
public:
  LocalClock(int64_t local_tick_now)
      : offset_(GetTimestamp<UNIT>() - local_tick_now) {}
