...
__global__ void AquireBlocks(VoxelMemPool voxel_mem, VoxelRGBW **voxel_blocks,
                             int *block_indics) {
...
__global__ void AssignBlocks(VoxelMemPool voxel_mem, int *block_indics,
                             int num_blocks) {
...
TEST_F(VoxelMemTest, Test1) {
  ...
  // allocation
  CUDA_SAFE_CALL(
      cudaMallocManaged(&voxel_blocks, sizeof(VoxelRGBW *) * NUM_BLOCK_ALLOC));
  CUDA_SAFE_CALL(
      cudaMemset(voxel_blocks, 0, sizeof(VoxelRGBW *) * NUM_BLOCK_ALLOC));
  CUDA_SAFE_CALL(
      cudaMallocManaged(&block_indics, sizeof(int) * NUM_BLOCK_ALLOC));
  ...
  AssignBlocks<<<1, THREAD_DIM>>>(voxel_mem_pool, block_indics,
                                  NUM_BLOCK_ALLOC);
  CUDA_CHECK_ERROR;
  for (int i = 0; i < NUM_BLOCK_ALLOC; ++i) {
    VoxelRGBW voxels[BLOCK_VOLUME];
    CUDA_SAFE_CALL(cudaMemcpy(voxels, voxel_blocks[i],
                              sizeof(VoxelRGBW) * BLOCK_VOLUME,
