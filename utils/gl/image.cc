...
void GLImageBase::BindImage(int height, int width, const void *data) {
  ...
  CUDA_SAFE_CALL(
      cudaGraphicsGLRegisterImage(&cuda_resrc_, texture_, GL_TEXTURE_2D,
...
void GLImageBase::LoadCuda(const void *data, cudaStream_t stream) {
  ...
  CUDA_SAFE_CALL(
      cudaGraphicsSubResourceGetMappedArray(&array, cuda_resrc_, 0, 0));
  CUDA_SAFE_CALL(cudaMemcpy2DToArrayAsync(
      array, 0, 0, data, width_ * ElementSize(), width_ * ElementSize(),
      height_, cudaMemcpyDeviceToDevice, stream));
