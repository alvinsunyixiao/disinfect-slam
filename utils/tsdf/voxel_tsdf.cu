...
__global__ static void check_bound_kernel(const VoxelHashTable hash_table,
  ...
  visible_mask[idx] =
      (block.idx >= 0 && voxel_grid.x >= volumn_grid.xmin &&
       voxel_grid.y >= volumn_grid.ymin && voxel_grid.z >= volumn_grid.zmin &&
...
__global__ static void download_tsdf_kernel(const VoxelHashTable hash_table,
  ...
  const Vector3<short> pos_grid =
      (block.position << BLOCK_LEN_BITS) + offset_grid;
...
__device__ static bool is_full_block_visible(const Vector3<short> &block_pos,
  ...
  return is_voxel_visible({x, y, z}, cam_P_world, cam_params, voxel_size) &&
         ...
         is_voxel_visible({x + b, y + b, z}, cam_P_world, cam_params,
                          voxel_size) &&
         is_voxel_visible({x, y, z + b}, cam_P_world, cam_params, voxel_size) &&
         is_voxel_visible({x + b, y, z + b}, cam_P_world, cam_params,
                          voxel_size) &&
         is_voxel_visible({x, y + b, z + b}, cam_P_world, cam_params,
                          voxel_size) &&
         is_voxel_visible({x + b, y + b, z + b}, cam_P_world, cam_params,
                          voxel_size);
}
...
check_visibility_kernel(const VoxelHashTable hash_table, const float voxel_size,
                        const float max_depth, const CameraParams cam_params,
                        const SE3<float> cam_P_world, int *visible_mask) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  const VoxelBlock &block = hash_table.GetBlock(idx);
  if (block.idx < 0) {
    visible_mask[idx] = 0;
  ...
  visible_mask[idx] = is_partial_block_visible(block.position, cam_P_world,
                                               cam_params, voxel_size);
}

__global__ static void
gather_visible_blocks_kernel(const VoxelHashTable hash_table,
                             const int *visible_mask, const int *visible_indics,
                             VoxelBlock *output) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (visible_mask[idx]) {
...
__global__ static void block_allocate_kernel(
    VoxelHashTable hash_table, const float *img_depth,
    const CameraParams cam_params, const SE3<float> cam_P_world,
    const SE3<float> world_P_cam, const float voxel_size, const float max_depth,
    const float truncation, float *img_depth_to_range) {
  ...
  const Vector3<float> ray_grid =
      2 * truncation * ray_dir_grid; // start -> end vector
  // DDA for finding ray / block intersection
  const int step_grid = ceilf(
      fmaxf(fmaxf(fabsf(ray_grid.x), fabsf(ray_grid.y)), fabsf(ray_grid.z)) /
      BLOCK_LEN);
...
__global__ static void tsdf_integrate_kernel(
    VoxelBlock *blocks, VoxelMemPool voxel_mem, const SE3<float> cam_P_world,
    const CameraParams cam_params, const int num_visible_blocks,
    const float max_depth, const float truncation, const float voxel_size,
    const uchar3 *img_rgb, const float *img_depth, const float *img_ht,
    const float *img_lt, const float *img_depth_to_range) {
  ...
  const Vector3<short> pos_grid_abs =
      (blocks[blockIdx.x].position << BLOCK_LEN_BITS) + pos_grid_rel;
  ...
  if (u >= 0 && u < cam_params.img_w && v >= 0 && v < cam_params.img_h) {
    ...
    if (sdf > -truncation) {
      ...
      VoxelTSDF &voxel_tsdf =
          voxel_mem.GetVoxel<VoxelTSDF>(idx, blocks[blockIdx.x]);
      VoxelRGBW &voxel_rgbw =
          voxel_mem.GetVoxel<VoxelRGBW>(idx, blocks[blockIdx.x]);
      VoxelSEGM &voxel_segm =
          voxel_mem.GetVoxel<VoxelSEGM>(idx, blocks[blockIdx.x]);
      // weight running average
      const float weight_new =
          (1 - depth / max_depth) * 4; // TODO(alvin): better weighting here
      ...
      voxel_tsdf.tsdf =
          (voxel_tsdf.tsdf * weight_old + tsdf * weight_new) / weight_combined;
      ...
      const float positive = expf((weight_old * logf(voxel_segm.probability) +
                                   weight_new * logf(img_ht[img_idx])) /
                                  weight_combined);
      const float negative =
          expf((weight_old * logf(1 - voxel_segm.probability) +
                weight_new * logf(img_lt[img_idx])) /
               weight_combined);
...
__global__ static void space_carving_kernel(VoxelHashTable hash_table,
  ...
  tsdf_abs[tx] =
      fabsf(hash_table.mem.GetVoxel<VoxelTSDF>(tx, blocks[blockIdx.x]).tsdf);
  tsdf_abs[tx2] =
      fabsf(hash_table.mem.GetVoxel<VoxelTSDF>(tx2, blocks[blockIdx.x]).tsdf);
// reduce min
#pragma unroll
...
__global__ static void
ray_cast_kernel(const VoxelHashTable hash_table, const CameraParams cam_params,
                const SE3<float> cam_P_world, const SE3<float> world_P_cam,
                const float step_size, const float max_depth,
                const float voxel_size, uchar4 *img_tsdf_rgba,
  ...
  VoxelBlock cache;
  float tsdf_prev =
      hash_table.Retrieve<VoxelTSDF>(pos_grid.round<short>(), cache).tsdf;
    ...
    if (tsdf_prev > 0 && tsdf_curr <= 0 && tsdf_prev - tsdf_curr <= 1.5) {
      ...
      while ((pos1_grid - pos2_grid).dot(pos1_grid - pos2_grid) > .1) {
        const float tsdf_mid =
            hash_table.Retrieve<VoxelTSDF>(pos_mid_grid.round<short>(), cache)
                .tsdf;
        if (tsdf_mid < 0) {
      ...
      const VoxelRGBW voxel_rgbw =
          hash_table.Retrieve<VoxelRGBW>(final_grid, cache);
      const VoxelSEGM voxel_segm =
          hash_table.Retrieve<VoxelSEGM>(final_grid, cache);
      // calculate gradient
      const Vector3<float> norm_raw_grid(
          hash_table
                  .Retrieve<VoxelTSDF>(
                      {final_grid.x + 1, final_grid.y, final_grid.z}, cache)
                  .tsdf -
              hash_table
                  .Retrieve<VoxelTSDF>(
                      {final_grid.x - 1, final_grid.y, final_grid.z}, cache)
          ...
          hash_table
                  .Retrieve<VoxelTSDF>(
                      {final_grid.x, final_grid.y + 1, final_grid.z}, cache)
                  .tsdf -
              hash_table
                  .Retrieve<VoxelTSDF>(
                      {final_grid.x, final_grid.y - 1, final_grid.z}, cache)
          ...
          hash_table
                  .Retrieve<VoxelTSDF>(
                      {final_grid.x, final_grid.y, final_grid.z + 1}, cache)
                  .tsdf -
              hash_table
                  .Retrieve<VoxelTSDF>(
                      {final_grid.x, final_grid.y, final_grid.z - 1}, cache)
      ...
      const float diffusivity =
          fmaxf(norm_raw_grid.dot(-ray_dir_world) /
                    sqrtf(norm_raw_grid.dot(norm_raw_grid)),
                0);
      const float alpha = fmaxf(voxel_segm.probability - 0.5, 0) / .5;
      img_tsdf_rgba[idx] = make_uchar4(
          alpha * 255 + (1 - alpha) * voxel_rgbw.rgb.x,
          (1 - alpha) * voxel_rgbw.rgb.y, (1 - alpha) * voxel_rgbw.rgb.z, 255);
      img_tsdf_normal[idx] =
...
TSDFGrid::TSDFGrid(float voxel_size, float truncation)
  ...
  CUDA_SAFE_CALL(cudaMalloc(&visible_indics_aux_,
                            sizeof(int) * NUM_ENTRY / (2 * SCAN_BLOCK_SIZE)));
  ...
  CUDA_SAFE_CALL(
      cudaMalloc(&img_depth_to_range_, sizeof(float) * MAX_IMG_SIZE));
...
void TSDFGrid::Integrate(const cv::Mat &img_rgb, const cv::Mat &img_depth,
  ...
  CUDA_SAFE_CALL(cudaMemcpyAsync(img_lt_, img_lt.data,
                                 ...
                                 cudaMemcpyHostToDevice, stream2_));
  // compute
  spdlog::debug("[TSDF] pre integrate: {} active blocks",
                hash_table_.NumActiveBlock());
  Allocate(img_rgb, img_depth, max_depth, cam_params, cam_P_world);
  const int num_visible_blocks =
      GatherVisible(max_depth, cam_params, cam_P_world);
  CUDA_SAFE_CALL(
      cudaStreamSynchronize(stream2_)); // synchronize ht / lt img copy
  ...
  spdlog::debug("[TSDF] post integrate: {} active blocks",
                hash_table_.NumActiveBlock());
}
...
void TSDFGrid::Allocate(const cv::Mat &img_rgb, const cv::Mat &img_depth,
                        ...
                        const SE3<float> &cam_P_world) {
  const dim3 IMG_BLOCK_DIM(ceil((float)cam_params.img_w / 32),
                           ceil((float)cam_params.img_h / 16));
...
int TSDFGrid::GatherVisible(float max_depth, const CameraParams &cam_params,
  ...
  check_visibility_kernel<<<GATHER_BLOCK_DIM, BLOCK_VOLUME, 0, stream_>>>(
      hash_table_, voxel_size_, max_depth, cam_params, cam_P_world,
      visible_mask_);
  CUDA_STREAM_CHECK_ERROR(stream_);
...
std::vector<VoxelSpatialTSDF> TSDFGrid::GatherValid() {
  spdlog::debug("[TSDF] {} active blocks before download",
                hash_table_.NumActiveBlock());
  ...
  CUDA_SAFE_CALL(cudaMemcpyAsync(ret.data(), voxel_pos_tsdf,
                                 sizeof(VoxelSpatialTSDF) * num_visible_blocks *
                                     BLOCK_VOLUME,
...
TSDFGrid::GatherVoxels(const BoundingCube<float> &volumn) {
...
int TSDFGrid::GatherBlock() {
  ...
  prefix_sum<int>(visible_mask_, visible_indics_, visible_indics_aux_,
                  NUM_ENTRY, stream_);
  // gather visible blocks into contiguous array
  gather_visible_blocks_kernel<<<GATHER_BLOCK_DIM, GATHER_THREAD_DIM, 0,
                                 stream_>>>(hash_table_, visible_mask_,
                                            visible_indics_, visible_blocks_);
  CUDA_STREAM_CHECK_ERROR(stream_);
  // get number of visible blocks from scanned index array
  int num_visible_blocks;
  CUDA_SAFE_CALL(cudaMemcpyAsync(&num_visible_blocks,
                                 visible_indics_ + NUM_ENTRY - 1, sizeof(int),
                                 cudaMemcpyDeviceToHost, stream_));
  CUDA_SAFE_CALL(cudaStreamSynchronize(stream_));
...
void TSDFGrid::UpdateTSDF(int num_visible_blocks, float max_depth,
  ...
  tsdf_integrate_kernel<<<num_visible_blocks, VOXEL_BLOCK_DIM, 0, stream_>>>(
      visible_blocks_, hash_table_.mem, cam_P_world, cam_params,
      num_visible_blocks, max_depth, truncation_, voxel_size_, img_rgb_,
      img_depth_, img_ht_, img_lt_, img_depth_to_range_);
  CUDA_STREAM_CHECK_ERROR(stream_);
}
...
void TSDFGrid::RayCast(float max_depth, const CameraParams &virtual_cam,
                       const SE3<float> &cam_P_world, GLImage8UC4 *tsdf_rgba,
  ...
  ray_cast_kernel<<<IMG_BLOCK_DIM, IMG_THREAD_DIM, 0, stream_>>>(
      ...
      truncation_ / 2, max_depth, voxel_size_, img_tsdf_rgba_,
      img_tsdf_normal_);
  CUDA_STREAM_CHECK_ERROR(stream_);
  if (tsdf_rgba) {
