...
void VoxelHashTable::ResetLocks(cudaStream_t stream) {
  reset_locks_kernel<<<NUM_BUCKET / 1024, 1024, 0, stream>>>(bucket_locks_,
                                                             NUM_BUCKET);
...
#pragma unroll
  for (int i = 0; i < NUM_ENTRY_PER_BUCKET; ++i) {
    ...
    if (block.idx < 0) {
      if (atomicExch(&bucket_locks_[bucket_idx], LOCKED) ==
          FREE) { // lock bucket
  ...
  const unsigned int bucket_idx_last =
      entry_idx_last >> NUM_ENTRY_PER_BUCKET_BITS;
  // append to list
  ...
  while (true) {
    ...
    if ((entry_idx_next & ENTRY_PER_BUCKET_MASK) != ENTRY_PER_BUCKET_MASK &&
      ...
      const unsigned int bucket_idx_next =
          entry_idx_next >> NUM_ENTRY_PER_BUCKET_BITS;
      if (atomicExch(&bucket_locks_[bucket_idx_last], LOCKED) ==
              FREE && // lock last bucket
          atomicExch(&bucket_locks_[bucket_idx_next], LOCKED) ==
              FREE) { // lock next bucket
        ...
        const unsigned int wrap =
            entry_idx_next > entry_idx_last ? 0 : NUM_ENTRY;
...
#pragma unroll
  ...
  if (block_head.position == block_pos && block_head.idx >= 0) {
    if (atomicExch(&bucket_locks_[bucket_idx], LOCKED) == FREE) {
      const unsigned int entry_idx_next =
          (entry_idx_last + block_head.offset) & ENTRY_MASK;
      VoxelBlock &block_next = hash_table_[entry_idx_next];
      mem.ReleaseBlock(block_head.idx);
      block_head.position = block_next.position;
      // check if reaches tail
      block_head.offset =
          block_next.offset ? block_head.offset + block_next.offset : 0;
  ...
  while (hash_table_[entry_idx_last].offset) {
    VoxelBlock &block_last = hash_table_[entry_idx_last];
    const unsigned int entry_idx_curr =
        (entry_idx_last + block_last.offset) & ENTRY_MASK;
    VoxelBlock &block_curr = hash_table_[entry_idx_curr];
    if (block_curr.position == block_pos && block_curr.idx >= 0) {
      if (atomicExch(&bucket_locks_[bucket_idx], LOCKED) ==
          FREE) { // lock original bucket
        // check if reaches tail
        block_last.offset =
            block_curr.offset ? block_last.offset + block_curr.offset : 0;
        // free current entry
...
__device__ float VoxelHashTable::RetrieveTSDF(const Vector3<float> &point,
  ...
  const float tsdf_000 =
      Retrieve<VoxelTSDF>(Vector3<short>(pl.x, pl.y, pl.z), cache).tsdf;
  const float tsdf_001 =
      Retrieve<VoxelTSDF>(Vector3<short>(ph.x, pl.y, ph.z), cache).tsdf;
  const float tsdf_010 =
      Retrieve<VoxelTSDF>(Vector3<short>(pl.x, ph.y, pl.z), cache).tsdf;
  const float tsdf_011 =
      Retrieve<VoxelTSDF>(Vector3<short>(pl.x, ph.y, ph.z), cache).tsdf;
  const float tsdf_100 =
      Retrieve<VoxelTSDF>(Vector3<short>(ph.x, pl.y, pl.z), cache).tsdf;
  const float tsdf_101 =
      Retrieve<VoxelTSDF>(Vector3<short>(ph.x, pl.y, ph.z), cache).tsdf;
  const float tsdf_110 =
      Retrieve<VoxelTSDF>(Vector3<short>(ph.x, ph.y, pl.z), cache).tsdf;
  const float tsdf_111 =
      Retrieve<VoxelTSDF>(Vector3<short>(ph.x, ph.y, ph.z), cache).tsdf;
