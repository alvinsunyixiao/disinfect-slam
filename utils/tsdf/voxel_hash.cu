...
__device__ float VoxelHashTable::RetrieveTSDF(const Eigen::Matrix<float, 3, 1>& point,
                                              VoxelBlock& cache) const {
  const Eigen::Matrix<float, 3, 1> pl = point.unaryExpr([] (const float x) { return floorf(x); });
  const Eigen::Matrix<float, 3, 1> ph = pl + Eigen::Matrix<float, 3, 1>::Ones();
  const Eigen::Matrix<float, 3, 1> alpha = ph - point;
