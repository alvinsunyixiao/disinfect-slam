...
public:
  ...
  __device__ __host__ Vector2<T>(const T &x_, const T &y_) : x(x_), y(y_) {}
...
template <typename T>
__device__ __host__ inline Vector2<T> operator+(const T &lhs,
                                                const Vector2<T> &rhs) {
  Vector2<T> ret(lhs);
  return ret += rhs;
}
...
public:
  ...
  __device__ __host__ inline Vector3<Tout> cast() const {
    return Vector3<Tout>(static_cast<Tout>(x), static_cast<Tout>(y),
                         static_cast<Tout>(z));
  }
...
template <typename T>
__device__ __host__ inline Vector3<T> operator+(const T &lhs,
                                                const Vector3<T> &rhs) {
  Vector3<T> ret(lhs);
  return ret += rhs;
}
...
public:
  ...
  __device__ __host__ Vector4<T>(const T &x_, const T &y_, const T &z_,
                                 const T &w_)
      : x(x_), y(y_), z(z_), w(w_) {}
...
template <typename T>
__device__ __host__ inline Vector4<T> operator+(const T &lhs,
                                                const Vector4<T> &rhs) {
  Vector4<T> ret(lhs);
  return ret += rhs;
}
