...
void ImuSyncer::AddMeasurement(const rs2::frame& frame) {
...
void ImuSyncer::TryInvokeSync() {
  ...
  if (gyro_q_.front().get_timestamp() < accel_q_.back().get_timestamp() &&
    ...
    if (it_next != gyro_q_.end()) {
      ...
      // interpolate gyro
      const auto gyro_start = Eigen::Map<Eigen::Vector3f>((float*)it_next->get_data());
      const auto gyro_end = Eigen::Map<Eigen::Vector3f>((float*)it_curr->get_data());
...
L515::L515() : align_to_color_(RS2_STREAM_COLOR) {
  ...
  cfg_.enable_stream(RS2_STREAM_COLOR, RGB_WIDTH, RGB_HEIGHT, rs2_format::RS2_FORMAT_RGB8, FPS);
...
int64_t L515::GetRGBDFrame(cv::Mat* color_img, cv::Mat* depth_img) const {
  ...
  const cv::Size img_size(DEPTH_WIDTH, DEPTH_HEIGHT);
...
void L515Async::HandleRawStream(const rs2::frame& frame) {
