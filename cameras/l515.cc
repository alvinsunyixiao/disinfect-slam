...
uint64_t L515::GetRGBDFrame(cv::Mat* color_img, cv::Mat* depth_img) const {
  const auto frame = GetRGBDFrame();
...
timed_rgbd_frame_t L515::GetRGBDFrame() const {
  ...
  const cv::Mat color_img(
      cv::Size(WIDTH, HEIGHT), CV_8UC3, (void*)color_frame.get_data(), cv::Mat::AUTO_STEP);
  const cv::Mat depth_img(
      cv::Size(WIDTH, HEIGHT), CV_16UC1, (void*)depth_frame.get_data(), cv::Mat::AUTO_STEP);

  timed_rgbd_frame_t frame;
  color_img.copyTo(frame.color_img);
  depth_img.convertTo(frame.depth_img, CV_32FC1, depth_scale_);
  ...
  frame.timestamp = depth_frame.get_frame_metadata(RS2_FRAME_METADATA_BACKEND_TIMESTAMP);
