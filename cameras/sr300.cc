...
SR300::SR300() : align_to_color_(RS2_STREAM_COLOR) {
  cfg_.enable_stream(RS2_STREAM_DEPTH, WIDTH, HEIGHT,
                     rs2_format::RS2_FORMAT_Z16, FPS);
  cfg_.enable_stream(RS2_STREAM_COLOR, WIDTH, HEIGHT,
                     rs2_format::RS2_FORMAT_RGB8, FPS);
...
void SR300::get_rgbd_frame(cv::Mat *color_img, cv::Mat *depth_img) const {
  ...
  *color_img = cv::Mat(cv::Size(WIDTH, HEIGHT), CV_8UC3,
                       (void *)frameset.get_color_frame().get_data(),
                       cv::Mat::AUTO_STEP);
  *depth_img = cv::Mat(cv::Size(WIDTH, HEIGHT), CV_16UC1,
                       (void *)frameset.get_depth_frame().get_data(),
                       cv::Mat::AUTO_STEP);
}

void SR300::set_depth_sensor_option(const rs2_option option,
                                    const float value) {
  auto sensor = pipe_profile_.get_device().first<rs2::depth_sensor>();
  ...
  if (value < option_range.min || value > option_range.max) {
    spdlog::error("value {} out of range ([{}, {}])", value, option_range.min,
                  option_range.max);
    return;
  }
  try {
