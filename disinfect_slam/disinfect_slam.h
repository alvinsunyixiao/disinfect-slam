...
class DISINFSystem {
public:
  DISINFSystem(std::string camera_config_path, std::string vocab_path,
               std::string seg_model_path, bool rendering_flag);

  ~DISINFSystem();

  void feed_rgbd_frame(const cv::Mat &img_rgb, const cv::Mat &img_depth,
                       int64_t timestamp);

  void feed_stereo_frame(const cv::Mat &img_left, const cv::Mat &img_right,
                         int64_t timestamp);
