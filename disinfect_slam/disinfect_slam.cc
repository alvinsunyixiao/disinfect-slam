...
DISINFSystem::DISINFSystem(std::string camera_config_path,
                           std::string vocab_path, std::string seg_model_path,
                           bool rendering_flag) {
  ...
  TSDF_ = std::make_shared<TSDFSystem>(
      0.01, 0.06, 4, GetIntrinsicsFromFile(camera_config_path),
  ...
  if (rendering_flag) {
    RENDERER_ = std::make_shared<ImageRenderer>("tsdf", SLAM_, TSDF_,
                                                camera_config_path);
  }
}
...
void DISINFSystem::feed_rgbd_frame(const cv::Mat &img_rgb,
  ...
  const SE3<float> posecam_P_world =
      camera_pose_manager_->query_pose(timestamp);
  ...
  TSDF_->Integrate(posecam_P_world, my_img_rgb, my_img_depth, prob_map[0],
                   prob_map[1]);
}
...
void DISINFSystem::feed_stereo_frame(const cv::Mat &img_left,
                                     ...
                                     int64_t timestamp) {
  const pose_valid_tuple m = SLAM_->feed_stereo_images_w_feedback(
      img_left, img_right, timestamp / 1e3);
  const SE3<float> posecam_P_world(
      m.first(0, 0), m.first(0, 1), m.first(0, 2), m.first(0, 3), m.first(1, 0),
      m.first(1, 1), m.first(1, 2), m.first(1, 3), m.first(2, 0), m.first(2, 1),
      m.first(2, 2), m.first(2, 3), m.first(3, 0), m.first(3, 1), m.first(3, 2),
      m.first(3, 3));
  if (m.second)
...
DISINFSystem::query_tsdf(const BoundingCube<float> &volumn) {
