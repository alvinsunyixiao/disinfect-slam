...
torch::Tensor mat_to_tensor(cv::Mat &my_mat) {
  ...
  // opencv format H*W*C
  auto input_tensor =
      torch::from_blob(my_mat.data, {1, my_mat.rows, my_mat.cols, 3});
...
cv::Mat float_tensor_to_float_mat(const torch::Tensor &my_tensor) {
  ...
  std::memcpy((void *)ret.data, temp_tensor.data_ptr(),
              sizeof(float) * temp_tensor.numel());
...
cv::Mat float_tensor_to_uint8_mat(const torch::Tensor &my_tensor) {
  ...
  // copy the data from out_tensor to resultImg
  std::memcpy((void *)ret.data, temp_tensor.data_ptr(),
              sizeof(uint8_t) * temp_tensor.numel());
  return ret;
}
...
std::vector<cv::Mat> inference_engine::infer_one(const cv::Mat &rgb_img,
  ...
  torch::Tensor downsized_rgb_img_tensor_cuda =
      downsized_rgb_img_tensor.to(torch::kCUDA);
