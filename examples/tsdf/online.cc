...
void reconstruct(const ZEDNative &zed_native, const L515 &l515,
  ...
  // initialize TSDF
  auto TSDF = std::make_shared<TSDFSystem>(
      0.01, 0.06, 4, GetIntrinsicsFromFile(config_file_path),
  ...
  std::thread t_slam([&]() {
    while (true) {
      ...
      // visual slam
      const pose_valid_tuple m = SLAM->feed_stereo_images_w_feedback(
  ...
  std::thread t_tsdf([&]() {
    while (true) {
      ...
      std::vector<cv::Mat> prob_map =
          segmentation_engine->infer_one(img_rgb, false);
      TSDF->Integrate(posecam_P_world, img_rgb, img_depth, prob_map[0],
                      prob_map[1]);
...
int main(int argc, char *argv[]) {
  ...
  auto seg_model_path = op.add<popl::Value<std::string>>(
      "m", "model", "PyTorch JIT traced model path");
