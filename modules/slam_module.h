...
public:
  ...
  unsigned int FeedStereoImages(const cv::Mat &img_left,
  ...
  //! (Note: Left and Right images must be stereo-rectified)
  pose_valid_tuple feed_stereo_images_w_feedback(
      const cv::Mat &left_img, const cv::Mat &right_img, const double timestamp,
      const cv::Mat &mask = cv::Mat{});

  //! Feed an RGBD frame to SLAM system and return current pose and valid flag
  //! (Note: RGB and Depth images must be aligned)
  pose_valid_tuple feed_RGBD_images_w_feedback(const cv::Mat &rgb_img,
