...
void ImageRenderer::DispatchInput() {
    ...
    if (!io.WantCaptureMouse && ImGui::IsMouseDragging(0) && tsdf_normal_.GetWidth()) {
        ...
        const Vector3<float> T = virtual_cam_T_world_old_.GetT();
        const Matrix3<float> R = virtual_cam_T_world_old_.GetR();
...
void ImageRenderer::Render() {
    ...
    if (follow_cam_) {
        ...
        virtual_cam_T_world_ = SE3<float>(cam_T_world_.GetR(),
                                        cam_T_world_.GetT() + Vector3<float>(0, 0, step));
    }
    // bounding cube query
